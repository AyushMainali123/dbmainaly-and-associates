...
interface OurImpactSectionProps {
    data?: {
        ...
        testimonial?: { quote?: string | null; authorInitials?: string | null; authorName?: string | null; authorRole?: string | null; } | null;
        image?: number | Media | null;
    };
}

export default function OurImpactSection({ data }: OurImpactSectionProps) {
    const imageUrl = data?.image && typeof data.image === 'object' && data.image.url
        ? data.image.url
            ...
            <div className="max-w-[1440px] mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-24 items-center">
                <div className="relative order-2 md:order-1">
                    ...
                    <div className="pl-8 md:pl-12 py-4">
                        <Small className="text-primary font-bold tracking-widest uppercase mb-6 block">{data?.badge || "Our Trajectory"}</Small>
                        <H2 className="mb-8">{data?.title || "Decades of Fiscal Authority."}</H2>
                        <P className="text-on-surface-variant text-lg md:text-xl mb-12 max-w-lg">
                            {data?.description || "Since our inception, we have been the standard-bearer for financial integrity in Nepal, guiding organizations from startup to public listing."}
                        </P>
                        <div className="grid grid-cols-2 gap-8 md:gap-12">
                            {data?.stats?.map((stat, i) => (
                                <div key={stat.id || i}>
                                    <H3 className="text-primary mb-1">{stat.value}</H3>
                                    <Small className="text-on-surface font-bold tracking-wide uppercase">{stat.label}</Small>
                                </div>
                            )) || (
                                    <>
                                        <div>
                    ...
                    <div className="relative h-full rounded-2xl overflow-hidden shadow-2xl">
                        ...
                        <div className="absolute bottom-6 left-6 right-6 md:bottom-10 md:left-10 md:right-10">
                            <div className="bg-surface-container-lowest/10 backdrop-blur-xl p-6 md:p-8 rounded-xl border border-white/20">
                                ...
                                <p className="text-white text-base md:text-lg italic mb-6">{data?.testimonial?.quote || '"DB Mainaly & Associates transformed our corporate governance structure, enabling our transition to a public entity."'}</p>
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-full bg-primary-fixed flex items-center justify-center font-bold text-primary shrink-0">{data?.testimonial?.authorInitials || 'SC'}</div>
                                    <div>
                                        <p className="text-white font-bold text-sm mb-1">{data?.testimonial?.authorName || 'Sunil Chaudhary'}</p>
                                        <Small className="text-white/60">{data?.testimonial?.authorRole || 'CEO, Apex Infrastructure'}</Small>
                                    </div>
                                </div>
                            </div>
                        </div>
